...
local PriceEntity = {}
PriceEntity.__index = PriceEntity


function PriceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PriceEntity)
  self._name = "price"
  self._client = client
...
function PriceEntity:get_name()
  return self._name
end


function PriceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PriceEntity.new(self._client, opts)
end
...
function PriceEntity:mark_deleted()
  self._deleted = true
end


function PriceEntity:deleted()
  return true == self._deleted
end


function PriceEntity:data_set(args)
  if args ~= nil then
...
function PriceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PriceEntity:match_set(args)
  if args ~= nil then
...
function PriceEntity:match_get()
...
function PriceEntity:stream(action, args, callopts)
...
---@param reqmatch PriceListMatch
---@param ctrl? table
---@return Price[]
---@return string? err
function PriceEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PriceEntity:_run_op(ctx, post_done)
  local utility = self._utility
