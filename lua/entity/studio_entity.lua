...
local StudioEntity = {}
StudioEntity.__index = StudioEntity


function StudioEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StudioEntity)
  self._name = "studio"
  self._client = client
...
function StudioEntity:get_name()
  return self._name
end


function StudioEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StudioEntity.new(self._client, opts)
end
...
function StudioEntity:mark_deleted()
  self._deleted = true
end


function StudioEntity:deleted()
  return true == self._deleted
end


function StudioEntity:data_set(args)
  if args ~= nil then
...
function StudioEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StudioEntity:match_set(args)
  if args ~= nil then
...
function StudioEntity:match_get()
...
function StudioEntity:stream(action, args, callopts)
...
function StudioEntity:_run_op(ctx, post_done)
  local utility = self._utility
