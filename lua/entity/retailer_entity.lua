...
local RetailerEntity = {}
RetailerEntity.__index = RetailerEntity


function RetailerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RetailerEntity)
  self._name = "retailer"
  self._client = client
...
function RetailerEntity:get_name()
  return self._name
end


function RetailerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RetailerEntity.new(self._client, opts)
end
...
function RetailerEntity:mark_deleted()
  self._deleted = true
end


function RetailerEntity:deleted()
  return true == self._deleted
end


function RetailerEntity:data_set(args)
  if args ~= nil then
...
function RetailerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RetailerEntity:match_set(args)
  if args ~= nil then
...
function RetailerEntity:match_get()
...
function RetailerEntity:stream(action, args, callopts)
...
function RetailerEntity:_run_op(ctx, post_done)
  local utility = self._utility
