...
local ConsoleEntity = {}
ConsoleEntity.__index = ConsoleEntity


function ConsoleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ConsoleEntity)
  self._name = "console"
  self._client = client
...
function ConsoleEntity:get_name()
  return self._name
end


function ConsoleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ConsoleEntity.new(self._client, opts)
end
...
function ConsoleEntity:mark_deleted()
  self._deleted = true
end


function ConsoleEntity:deleted()
  return true == self._deleted
end


function ConsoleEntity:data_set(args)
  if args ~= nil then
...
function ConsoleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ConsoleEntity:match_set(args)
  if args ~= nil then
...
function ConsoleEntity:match_get()
...
function ConsoleEntity:stream(action, args, callopts)
...
function ConsoleEntity:_run_op(ctx, post_done)
  local utility = self._utility
