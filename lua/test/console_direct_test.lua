...
describe("ConsoleDirect", function()
  it("should direct-list-console", function()
    local setup = console_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-console", setup.live and "live" or "unit")
  ...
  it("should direct-load-console", function()
    local setup = console_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-console", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function console_direct_setup(mockres)
