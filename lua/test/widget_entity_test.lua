...
describe("WidgetEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NEXARDA_TEST_WIDGET_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local widget_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.widget")))
    local widget_ref01_data = nil
    if #widget_ref01_data_raw > 0 then
      widget_ref01_data = helpers.to_map(widget_ref01_data_raw[1][2])
    end

    -- LOAD
    local widget_ref01_ent = client:Widget(nil)
    local widget_ref01_match_dt0 = {}
    local widget_ref01_data_dt0_loaded, err = widget_ref01_ent:load(widget_ref01_match_dt0, nil)
    assert.is_nil(err)
    assert.is_not_nil(widget_ref01_data_dt0_loaded)
...
function widget_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("NEXARDA_TEST_WIDGET_ENTID")
