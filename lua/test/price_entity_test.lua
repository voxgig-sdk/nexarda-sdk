...
describe("PriceEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Price(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Price(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NEXARDA_TEST_PRICE_ENTID JSON to run live")
      return
    end
    ...
    local price_ref01_match = {
      ["console_id"] = setup.idmap["console01"],
    }

    local price_ref01_list_result, err = price_ref01_ent:list(price_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(price_ref01_list_result)
...
function price_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("NEXARDA_TEST_PRICE_ENTID")
