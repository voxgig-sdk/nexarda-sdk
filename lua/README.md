...
The Lua SDK for the Nexarda API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Console(nil):list(nil, nil)
...
```lua
local result, err = client:Console(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:Nexarda(nil):load(
  { id = "test01" }, nil
)
