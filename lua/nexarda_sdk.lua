...
-- by passing it.
function NexardaSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function NexardaSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "NexardaSDK: graphql: " .. msg
    res.graphql = errors
  end
