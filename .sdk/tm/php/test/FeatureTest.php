...
class FtCtrl extends NexardaControl
{
...
class FtClient
{
    ...
    private NexardaUtility $utility;

    public function __construct(NexardaUtility $utility, array $options)
    {
    ...
    public function get_utility(): NexardaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NexardaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NexardaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NexardaConfig::make_feature($name);
    ...
    private static function buildUrl(NexardaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NexardaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NexardaResponse($fetched);
        }
    ...
    private function fail(NexardaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NexardaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
