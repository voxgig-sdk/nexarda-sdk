...
class StudioEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Studio(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Studio(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    studio_ref01_data_dt0_load_result = Helpers.to_map(studio_ref01_data_dt0_loaded.respond_to?(:data_get) ? studio_ref01_data_dt0_loaded.data_get : studio_ref01_data_dt0_loaded)
