...
class ConsoleEntity
  ...
  def make
    opts = @_entopts.dup
    ConsoleEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NexardaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Console, Hash] the current Console data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NexardaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Console fields)
  def match_get
  ...
  #
  # @param reqmatch [ConsoleLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Console.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Console, Hash] the loaded Console; raises NexardaError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NexardaHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [ConsoleListMatch, Hash, nil] match filter (any subset of
  #   Console fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Console>, Array] the matching Console items; raises NexardaError on failure
