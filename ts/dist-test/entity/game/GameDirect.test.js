...
(0, node_test_1.describe)('GameDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-game', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-game', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["platform01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.platform_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.platform_id = candidateId;
            params.platform_id = setup.idmap['platform01'];
        }
        else {
            params.platform_id = 'direct01';
        }
        const result = await client.direct({
            path: 'games/platform/{platform_id}',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['NEXARDA_TEST_GAME_ENTID'];
