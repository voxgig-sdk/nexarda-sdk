...
//     (\`new NexardaSDK(...)\` / \`NexardaSDK.test(...)\` become
//     \`NexardaSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new NexardaSDK(...)\` and any
// \`NexardaSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NexardaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NexardaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
