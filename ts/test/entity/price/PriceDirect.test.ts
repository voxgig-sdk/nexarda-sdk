...
describe('PriceDirect', async () => {
  ...
  test('direct-list-price', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-price', setup.live)) return
    if (skipIfMissingIds(t, setup, ["game01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NEXARDA_TEST_PRICE_ENTID']
