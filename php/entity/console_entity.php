...
class ConsoleEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ConsoleEntity($this->_client, $opts);
    }

    /**
     * @param Console|array $args Console data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NexardaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Console|array The current Console data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Console fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NexardaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Console fields).
     */
    ...
    /**
     * Load a single Console.
     *
     * @param ConsoleLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ConsoleLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Console|array The loaded Console as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = NexardaHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Console items matching the given filter.
     *
     * @param ConsoleListMatch|array|null $reqmatch Match filter (any subset
     *   of Console fields) as an assoc-array; ConsoleListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Console[]|array A list of Console items as assoc-arrays at
