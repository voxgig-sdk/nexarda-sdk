...
class NexardaMakeError
{
    public static function call(?NexardaContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new NexardaContext([], null);
        }
        $op = $ctx->op ?? new NexardaOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new NexardaResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof NexardaError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new NexardaError('', $msg, $ctx);
        ...
        if ($err instanceof NexardaError) {
