...
print_r(array_map(fn($item) => $item->data_get(), $studio));
```
...
| `specifications` | `array` | Technical specifications |
...
| `location` | `array` | Studio location |
...
$widget = $client->Widget()->load(["product_id" => "product_id"]);
```
