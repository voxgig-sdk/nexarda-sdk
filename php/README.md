...
The PHP SDK for the Nexarda API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Console(null)->list(null, null);
...
```php
[$result, $err] = $client->Console(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->Nexarda(null)->load(
    ["id" => "test01"], null
);
