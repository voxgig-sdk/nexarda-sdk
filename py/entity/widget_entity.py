...
class WidgetEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return WidgetEntity(self._client, opts)
    ...
    def match_get(self) -> Widget:
        self._utility.feature_hook(self._entctx, "GetMatch")
        return vs.clone(self._match)

    
    def load(self, reqmatch=None, ctrl=None) -> Widget:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Widget().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
