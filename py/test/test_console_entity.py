...
class TestConsoleEntity:

    def test_should_create_instance(self):
        testsdk = NexardaSDK.test(None, None)
        ent = testsdk.Console(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Console(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Console(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _console_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "console." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NEXARDA_TEST_CONSOLE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        console_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.console")))
        console_ref01_data = None
        if len(console_ref01_data_raw) > 0:
        ...
        console_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(console_ref01_data_dt0_loaded))
        assert console_ref01_data_dt0_load_result is not None
        assert console_ref01_data_dt0_load_result["id"] == console_ref01_data["id"]



def _console_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/console/ConsoleTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("NEXARDA_TEST_CONSOLE_ENTID"))
