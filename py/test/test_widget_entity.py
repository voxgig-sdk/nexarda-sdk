    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NEXARDA_TEST_WIDGET_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        widget_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.widget")))
        widget_ref01_data = None
        if len(widget_ref01_data_raw) > 0:
            widget_ref01_data = helpers.to_map(widget_ref01_data_raw[0][1])

        # LOAD
        widget_ref01_ent = client.Widget(None)
        widget_ref01_match_dt0 = {}
        widget_ref01_data_dt0_loaded, err = widget_ref01_ent.load(widget_ref01_match_dt0, None)
        assert err is None
        assert widget_ref01_data_dt0_loaded is not None
...
def _widget_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("NEXARDA_TEST_WIDGET_ENTID"))
