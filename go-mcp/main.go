// Command Nexarda-mcp is an MCP server exposing the Nexarda
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Nexarda_list  — list records of an entity
//	Nexarda_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: NEXARDA_APIKEY carries the API key and
	// NEXARDA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NEXARDA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NEXARDA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNexardaSDK(opts)
