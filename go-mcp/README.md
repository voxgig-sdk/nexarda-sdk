...
[MCP](https://modelcontextprotocol.io) server exposing the Nexarda SDK as
two agent tools — `nexarda_list` and `nexarda_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nexarda-mcp)
make build

# 2. Provide credentials via the environment
export NEXARDA_APIKEY=sk_live_xxx
...
```jsonc
// nexarda_list: first page of records
{ "entity": "console" }
{ "entity": "console", "query": { } }

// nexarda_load: one record by id
{ "entity": "console", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nexarda-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nexarda \
     -- "$PWD"/dist/*/nexarda-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nexarda_list` and `nexarda_load` tools now appear
   in new sessions. Ask the agent to *"list console using nexarda"*
   and it calls `nexarda_list` with `{"entity":"console"}`.
...
```sh
export NEXARDA_APIKEY=sk_live_xxx            # API key
export NEXARDA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `nexarda_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nexarda_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NEXARDA_APIKEY` | API key sent with every request. |
| `NEXARDA_BASE` | Optional override of the API base URL. |
