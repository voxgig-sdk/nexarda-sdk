...
boru-driven command-line client **and** interactive REPL for the Nexarda
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nexarda-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nexarda-cli
   ```
...
```sh
./nexarda-cli load 1 console          # scalar shorthand for {id:1}
./nexarda-cli load '{id:1}' console     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `nexarda>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `console`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nexarda-cli`. |
...
(`./nexarda-cli <expr>`) and interactively (the REPL), and expressions compose the
