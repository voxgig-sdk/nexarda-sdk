...
func TestStudioDirect(t *testing.T) {
	t.Run("direct-list-studio", func(t *testing.T) {
		setup := studioDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-studio", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-studio", func(t *testing.T) {
		setup := studioDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-studio", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type studioDirectSetupResult struct {
...
func studioDirectSetup(mockres any) *studioDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["NEXARDA_TEST_STUDIO_ENTID"]; ok {
		...
		return &studioDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &studioDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
