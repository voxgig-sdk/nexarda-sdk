...
func TestWidgetEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		widgetRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.widget", setup.data)))
		var widgetRef01Data map[string]any
		if len(widgetRef01DataRaw) > 0 {
			widgetRef01Data = core.ToMapAny(widgetRef01DataRaw[0][1])
		}
		...
		widgetRef01MatchDt0 := map[string]any{}
		widgetRef01DataDt0Loaded, err := widgetRef01Ent.Load(widgetRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		if widgetRef01DataDt0Loaded == nil {
...
func widgetBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"widget01", "widget02", "widget03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("NEXARDA_TEST_WIDGET_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NEXARDA_TEST_WIDGET_ENTID"])
