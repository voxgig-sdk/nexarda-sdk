...
import (
	...
	omni "github.com/voxgig-sdk/nexarda-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NexardaError); is {
				out := map[string]any{
					"name":    "NexardaError",
...
func omniCtx(arg any, client *sdk.NexardaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NexardaSDK); is {
			client = live
