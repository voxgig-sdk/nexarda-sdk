...
func TestRetailerEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Retailer(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Retailer(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		retailerRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.retailer")))
